...
private static String extractJson(String raw) {
    ...
    if (objSt >= 0 && objEd > objSt) return s.substring(objSt, objEd + 1).trim();
    int arrSt = s.indexOf('['), arrEd = s.lastIndexOf(']');
    if (arrSt >= 0 && arrEd > arrSt) return s.substring(arrSt, arrEd + 1).trim();
    return s;
}
```
...
### 5-2. 배치 중 Gemini API 429 오류로 수집이 중단되는 문제

#### 상황
기사가 많을 때 Gemini API를 연속 호출하면서 429 오류가 발생했고, 이후 기사 요약이 모두 실패했습니다.

#### 해결
실패할수록 대기시간을 2배씩 늘려서 재시도하도록 구현했습니다.  
재시도 한도를 초과하면 해당 기사만 스킵하고 배치를 계속 진행합니다.

#### 배운 점
외부 API 호출이 포함된 배치는 개별 실패가 전체에 영향을 주지 않도록 설계해야 합니다.  
속도 제한이 있는 API는 호출 간격 제어와 재시도 전략을 초기부터 고려하는 것이 중요합니다.
